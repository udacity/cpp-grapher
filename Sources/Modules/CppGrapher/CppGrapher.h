...
public:
	...
	/// The traditional C-style practice of using argv + argc is an example of a "buffer + length" antipattern.
	...
	/// @param args[in]         a const vector of strings representing the arguments given to the program.  Note: the
	///                         system always provides the application's name as args[0].
...
private:
	void ValidateArgs( const std::vector<std::string> args );
};
